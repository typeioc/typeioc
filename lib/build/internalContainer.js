...
var InternalContainer = (function () {
    function InternalContainer(_registrationStorageService, _disposableStorageService, _registrationBaseService, _containerApiService) {
        ...
        this.children = [];
        this._dependencyScope = 1 /* None */;
        this._dependencyOwner = 2 /* Externals */;
    ...
    InternalContainer.prototype.createDependenciesRegistration = function (api) {
        ...
        }).filter(function (item) { return item.implementation || item.dependency.required === true ? true : false; });
        if (items.length !== api.dependenciesValue.length)
            return [];
        return items.map(function (item) {
            var baseRegistration = item.dependency.required === true ? _this.createRegistration(item.dependency.service) : item.implementation.cloneFor(_this);
