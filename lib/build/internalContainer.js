...
class InternalContainer {
    constructor(_registrationStorageService, _disposableStorageService, _registrationBaseService, _containerApiService, _invokerService, _resolutionDetails) {
        ...
        this._invokerService = _invokerService;
    ...
    createChild() {
        var child = new InternalContainer(this._registrationStorageService, this._disposableStorageService, this._registrationBaseService, this._containerApiService, this._invokerService, this._resolutionDetails);
        child.parent = this;
    ...
    registerImpl(registration) {
        if (!registration.factory &&
            ...
            var exception = new exceptions_1.NullReferenceError("Factory/Type/Value is not defined");
    ...
    createDependenciesRegistration(api) {
        const items = api.dependenciesValue.map(dependency => {
            ...
            if (!dependency.factory &&
                ...
                const exception = new exceptions_1.ResolutionError('Factory/Type/Value should be defined');
                exception.data = dependency;
                throw exception;
            }
